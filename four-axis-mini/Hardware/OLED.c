...
/**
  ...
  * @param  Line 起始行位置，范围：1~4
  * @param  Column 起始列位置，范围：1~16
  * @param  SingleChinese 要显示的汉字，只能写1个汉字
  * @retval 无
  */
void OLED_ShowSingleChinese(uint8_t Line, uint8_t Column, char *SingleChinese)
{
	...
	/*寻找索引*/
	for (index = 0; strcmp(OLED_CF16x16[index].Index, "") != 0; index ++)
	{
		if (strcmp(OLED_CF16x16[index].Index, SingleChinese) == 0)
		{
			break;
		}
	}
	...
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_CF16x16[index].Data[i]);			//显示上半部分内容
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);		//设置光标位置在下半部分
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_CF16x16[index].Data[i + 16]);		//显示下半部分内容
	}
}
...
/**
  ...
  * @param  Line 起始行位置，范围：1~4
  * @param  Column 起始列位置，范围：1~16
  * @param  SingleChinese 要显示的汉字，可以写多个
  * @retval 无
  */
